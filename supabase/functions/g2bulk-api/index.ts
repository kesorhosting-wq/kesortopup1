...
serve(async (req) => {
  ...
  try {
    ...
    if (!apiConfig.is_enabled) {
      return new Response(JSON.stringify({ success: false, error: "G2Bulk API is disabled." }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!apiKey) {
      return new Response(JSON.stringify({ success: false, error: "G2Bulk API key not configured." }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    switch (action) {
      ...
      case "get_category_products": {
        ...
        if (!category_id) {
          return new Response(JSON.stringify({ success: false, error: "category_id is required" }), {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
      ...
      case "get_game_catalogue": {
        ...
        if (!game_code) {
          return new Response(JSON.stringify({ success: false, error: "game_code is required" }), {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
      ...
      case "check_player_id": {
        ...
        if (!game_code || !user_id) {
          return new Response(JSON.stringify({ success: false, error: "game_code and user_id are required" }), {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
      ...
      case "check_order_status": {
        ...
        if (!order_id || !game_code) {
          return new Response(JSON.stringify({ success: false, error: "order_id and game_code are required" }), {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
      ...
      case "sync_products": {
        ...
        if (allProducts.length > 0) {
          const { error: upsertError } = await supabase.from("g2bulk_products").upsert(allProducts, {
      ...
      case "bulk_import_all": {
        ...
        console.log(
          `[G2Bulk-API] Starting bulk import with ${price_markup_percent}% markup, update_prices=${update_existing_prices}, selected_games=${selected_game_codes?.length || "all"}...`,
        );
        ...
        const { data: existingGames } = await supabase.from("games").select("id, name, g2bulk_category_id");

        const existingGameNames = new Set((existingGames || []).map((g) => g.name.toLowerCase()));
        const existingCategoryIds = new Set(
          (existingGames || []).filter((g) => g.g2bulk_category_id).map((g) => g.g2bulk_category_id),
        );
        const gameIdByCode = new Map(
          (existingGames || []).filter((g) => g.g2bulk_category_id).map((g) => [g.g2bulk_category_id, g.id]),
        );
        ...
        const packageByProductId = new Map(
          (existingPackages || []).map((p) => [p.g2bulk_product_id, { id: p.id, price: p.price }]),
        );
        ...
        const gamesToProcess =
          selected_game_codes && selected_game_codes.length > 0
                  ...
                  if (update_existing_prices) {
                    ...
                    if (existingPkg && Number(existingPkg.price) !== finalPrice) {
                      ...
                      if (!updateError) {
                        packagesUpdated++;
                        console.log(
                          `[G2Bulk-API] Updated price for ${catalogue.name}: ${existingPkg.price} -> ${finalPrice}`,
                        );
                      }
                    }
                  }
        ...
        console.log(
          `[G2Bulk-API] Bulk import complete: ${gamesCreated} games, ${packagesCreated} packages created, ${packagesUpdated} prices updated`,
        );
    ...
    if (responseData.success === false) {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error: responseData.message || responseData.detail || "API request failed",
          data,
        }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
